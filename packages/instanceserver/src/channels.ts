...
const handleInstance = async (
  ...
  logger.info('Initialized new instanceserver.')
...
const loadInstanceserver = async (
  app: Application,
  status: InstanceserverStatus,
  locationId: string,
  channelId: string,
  ...
  logger.info('Creating new instanceserver or updating current one.')
  ...
  /**
   * Since local environments do not have the ability to run multiple instanceservers,
...
const shutdownInstanceserver = async (app: Application, instanceId: string) => {
...
const handleUserDisconnect = async (app: Application, connection, user, instanceId) => {
  ...
  const activeUsersCount = getActiveUsersCount(user)
  if (activeUsersCount < 1) await shutdownInstanceserver(app, instanceId)
}
...
const onConnection = (app: Application) => async (connection: SocketIOConnectionType) => {
  ...
  await loadInstanceserver(app, status, locationId, channelId, sceneId, userId)
...
export default (app: Application): void => {
  ...
  app.service('instanceserver-load').on('patched', async (params) => {
    ...
    loadInstanceserver(app, status, locationId, null!, sceneId, null!)
  })
