...
export const setupSubdomain = async (transport: SocketWebRTCServerTransport) => {
  ...
  if (config.kubernetes.enabled) {
    await cleanupOldInstanceservers(transport)
...
export async function getFreeSubdomain(
  ...
  const subdomainResult = await transport.app.service('instanceserver-subdomain-provision').find({
    query: {
      is_number: stringSubdomainNumber
    }
  })
  if ((subdomainResult as any).total === 0) {
    await transport.app.service('instanceserver-subdomain-provision').create({
      allocated: true,
      is_number: stringSubdomainNumber,
      is_id: isIdentifier
    })
    ...
    const newSubdomainResult = (await transport.app.service('instanceserver-subdomain-provision').find({
      query: {
        is_number: stringSubdomainNumber
      }
    })) as any
    if (newSubdomainResult.total > 0 && newSubdomainResult.data[0].is_id === isIdentifier) return stringSubdomainNumber
    else return getFreeSubdomain(transport, isIdentifier, subdomainNumber + 1)
  } else {
    const subdomain = (subdomainResult as any).data[0]
    if (subdomain.allocated === true || subdomain.allocated === 1) {
      return getFreeSubdomain(transport, isIdentifier, subdomainNumber + 1)
    }
    await transport.app.service('instanceserver-subdomain-provision').patch(subdomain.id, {
      allocated: true,
      is_id: isIdentifier
    })
...
export async function cleanupOldInstanceservers(transport: SocketWebRTCServerTransport): Promise<void> {
  ...
  const instanceservers = await transport.app.k8AgonesClient.listNamespacedCustomObject(
    'agones.dev',
    'v1',
    'default',
    'gameservers'
  ...
  await transport.app.service('instanceserver-subdomain-provision').patch(
    null,
    {
      allocated: false
    },
